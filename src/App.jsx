...
function MainApp() {
  ...
  const [editing,    setEditing]    = useState(null); // 編集モード中のID
  ...
  const addText = ()=>{
    ...
    setElements(e=>[...e,el]);
    setSelected(el.id);
    setEditing(el.id); // 新規追加時は編集モードで開く
  };
  ...
  return (
    <div style={{ minHeight:"100vh", background:C.cream, fontFamily:"'Noto Sans JP',sans-serif", color:C.ink, paddingBottom:60 }}>
      ...
      <style>{`@keyframes spin{to{transform:rotate(360deg)}} @keyframes fadeUp{from{opacity:0;transform:translateY(6px)}to{opacity:1;transform:none}} *{box-sizing:border-box} ::-webkit-scrollbar{display:none} input::placeholder{color:#C0B8B0} textarea::placeholder{color:#C0B8B0}`}</style>
    </div>
  );
}
  ...
  return (
    <div style={{ maxWidth:520, margin:"0 auto", padding:"12px 16px 40px" }}>
      ...
      <p style={{ textAlign:"center", fontSize:10, color:C.gray, marginTop:5 }}>
        {selected ? "ドラッグで移動　ピンチで拡縮" : "↓ レイヤーで要素を選択してください"}
      </p>
                  ...
                  <div style={{ padding:"14px", background:`${C.g1}08`, borderTop:`1px solid ${C.g1}30`, animation:"fadeUp 0.2s ease" }}>
                    ...
                    <div style={{ display:"flex", flexDirection:"column", gap:8, marginBottom:14 }}>
                      <div style={{ display:"flex", alignItems:"center", gap:8 }}>
                        <input type="checkbox" id={`shadow_${el.id}`} checked={el.shadow} onChange={e=>updateEl(el.id,{shadow:e.target.checked})} />
                        <label htmlFor={`shadow_${el.id}`} style={{ fontSize:13, cursor:"pointer" }}>ドロップシャドウ</label>
                      </div>
                      <div style={{ display:"flex", alignItems:"center", gap:8, flexWrap:"wrap" }}>
                        <input type="checkbox" id={`outline_${el.id}`} checked={el.outline} onChange={e=>updateEl(el.id,{outline:e.target.checked})} />
                        <label htmlFor={`outline_${el.id}`} style={{ fontSize:13, cursor:"pointer" }}>縁取り</label>
                        {el.outline&&<>
                      ...
                      <div style={{ display:"flex", alignItems:"center", gap:8, flexWrap:"wrap" }}>
                        <input type="checkbox" id={`glow_${el.id}`} checked={el.glow} onChange={e=>updateEl(el.id,{glow:e.target.checked})} />
                        <label htmlFor={`glow_${el.id}`} style={{ fontSize:13, cursor:"pointer" }}>外光（グロー）</label>
